...
int encode(uint8_t *dst, uint8_t len) {
    ...
    // Message to encode: LYNX19GW_NF_NR_SENSORS_1_FRAME_ID
    {
        ...
        tmp.brake = lynx19gw_nf_nr_sensors_1_brake_encode(msg.brake());
        tmp.throttle = lynx19gw_nf_nr_sensors_1_throttle_encode(msg.throttle());
        // The following statement packs the encoded values into a CAN frame.
        int size = lynx19gw_nf_nr_sensors_1_pack(dst, &tmp, len);
        return size;
    }
    ...
    // Message to encode: LYNX19GW_NF_DL_BUTTONS_RTD_FRAME_ID
    {
        ...
        tmp.knob_l = lynx19gw_nf_dl_buttons_rtd_knob_l_encode(msg.knobL());
        tmp.knob_r = lynx19gw_nf_dl_buttons_rtd_knob_r_encode(msg.knobR());
        // The following statement packs the encoded values into a CAN frame.
        int size = lynx19gw_nf_dl_buttons_rtd_pack(dst, &tmp, len);
        return size;
    }
    ...
    // Message to encode: LYNX19GW_PER_PDO_1_TX_FRAME_ID
    {
        ...
        tmp.drive_engaged_right = lynx19gw_per_pdo_1_tx_drive_engaged_right_encode(msg.driveEngagedRight());
        tmp.torque_estimate_right = lynx19gw_per_pdo_1_tx_torque_estimate_right_encode(msg.torqueEstimateRight());
        tmp.speed_estimate_right = lynx19gw_per_pdo_1_tx_speed_estimate_right_encode(msg.speedEstimateRight());
        tmp.errors_right = lynx19gw_per_pdo_1_tx_errors_right_encode(msg.errorsRight());
        // The following statement packs the encoded values into a CAN frame.
        int size = lynx19gw_per_pdo_1_tx_pack(dst, &tmp, len);
        return size;
    }
    // Message to encode: LYNX19GW_AS_DL_SENSORS_FRAME_ID
    {
        ...
        memset(&tmp, 0, sizeof(tmp));
        // The following msg would have to be passed to this encoder externally.
        opendlv::cfsdProxyCANWriting::ASStatus msg;
    ...
    // Message to encode: LYNX19GW_PEL_PDO_1_TX_FRAME_ID
    {
        ...
        tmp.drive_engaged_left = lynx19gw_pel_pdo_1_tx_drive_engaged_left_encode(msg.driveEngagedLeft());
        tmp.torque_estimate_left = lynx19gw_pel_pdo_1_tx_torque_estimate_left_encode(msg.torqueEstimateLeft());
        tmp.speed_estimate_left = lynx19gw_pel_pdo_1_tx_speed_estimate_left_encode(msg.speedEstimateLeft());
        tmp.errors_left = lynx19gw_pel_pdo_1_tx_errors_left_encode(msg.errorsLeft());
        // The following statement packs the encoded values into a CAN frame.
        int size = lynx19gw_pel_pdo_1_tx_pack(dst, &tmp, len);
        return size;
    }
}
